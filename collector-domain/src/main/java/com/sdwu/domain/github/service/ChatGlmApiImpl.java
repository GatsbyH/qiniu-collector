...
public class ChatGlmApiImpl implements IChatGlmApi{
    ...
    public String getCountryByUserRelations(List<String> followersLocations) throws JsonProcessingException {
        List<ChatMessage> messages = new ArrayList<>();
        ChatMessage chatMessage = new ChatMessage(ChatMessageRole.USER.value(), followersLocations+"根据用户关注和粉丝的国家，猜测用户的国家，只需回答置信度最高的国家和它置信度，置信度为1-100%的数值，置信度低的数据为 N/A 值，最后只返回这两个值，不需要说明\n");
        messages.add(chatMessage);
        String requestId = String.format(requestIdTemplate, System.currentTimeMillis());
